...
OBSTACLES_2 = [(0.55, 0.20), (1.45, 0.20)]

mesh_data = io.gmsh.read_from_msh("dfg_benchmark_2obstacles.msh", comm, gdim=2)
...
# ================================================================
# CoV COMPUTATION (sampling + trapezoidal quadrature)
# ================================================================
...
def solve_stokes(method: str):
    ...
    problem = LinearProblem(a, L, bcs=bcs, kind="nest", P=P,
                            petsc_options=opts, petsc_options_prefix=pfx)
    uh, ph = problem.solve()
...
# HIGHLY VISCOUS PARAMETERS - SAME FOR ALL CASES!
...
if comm.rank == 0:
    ...
    print(f"Pressure drop per element: {(p_in - p_out) / 2:.6f} Pa")
    if cov_outlet is not None:
        ...
        print("Outlet CoV: could not be computed")
